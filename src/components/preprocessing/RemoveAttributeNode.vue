<template>
  <div>
    <a-dropdown :trigger="['contextmenu']" :disabled="!showContextMenu">
      ...
      <template #overlay>
        <a-menu>
          <a-menu-item @click="runRemoveAttr" class="my-menu-item">
            <template #icon>
  ...
  methods: {
    async submitForm() {
      ...
      const postData = {
        ...
        columnsToRemove: formState.removeIndices,
      }
      const response = await fetch("http://localhost:8081/files/removeAttribute", {
        method: "POST",
